(function () {
  var API = '/api/reportes/reporte-respuesta';
  ...
  function exportar(){
    ...
    var csv = [['Endpoint','Metodo','Prom(ms)','P95(ms)','P99(ms)','Min(ms)','Max(ms)','Fecha']].concat(filas)
      .map(function(row){ return row.join(','); })
      .join('\n');
    var blob = new Blob([csv], {type:'text/csv;charset=utf-8;'});
    var url = URL.createObjectURL(blob), a = document.createElement('a');
    a.href = url; a.download = 'ReporteRespuesta.csv'; a.click();
  }
